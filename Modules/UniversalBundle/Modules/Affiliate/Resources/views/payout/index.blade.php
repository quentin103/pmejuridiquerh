...
@endphp
    ...
    <div class="content-wrapper">
        ...
        @if (!$users->isEmpty() && $addPayoutPermission == 'all')
            <div class="d-flex">
                <div id="table-actions" class="flex-grow-1 align-items-center">
                    <x-forms.link-primary :link="route('payout.create')" class="mr-3 float-left openRightModal"
                        icon="plus">
                        @lang('affiliate::app.addNewWithdrawal')
...
"use strict";  // Enforces strict mode for the entire script
        $('#payout-table').on('preXhr.dt', function(e, settings, data) {
            var searchText = $('#search-text-field').val();
            data['searchText'] = searchText;
        });

        const showTable = () => {
            window.LaravelDataTables["payout-table"].draw();
        }
        ...
        $('body').on('change', '.change-payout-status', function() {
            ...
            if ($(this).val() == 'paid') {
                    $(this).closest('.select-picker').selectpicker('val', 'pending');
                    var url = "{{ route('payouts.confirm_paid', ':id') }}";
                    var id = $(this).data('payout-id');
                    url = url.replace(':id', id);
